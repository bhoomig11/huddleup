...
import { getUserUpcomingBookings } from "~/api/user";
import { data } from "react-router";
import type { BookingSummary } from "~/types/booking";
import type { Route } from "./+types/upcoming-bookings";
...
export async function clientLoader({ params }: Route.ClientLoaderArgs) {
  ...
  const response = await getUserUpcomingBookings(username);
  if (!response.ok) {
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-row">
        ...
        <div className="min-h-screen basis-3/4 p-10">
          <div className="space-y-6">
            <h1 className="text-lg font-bold text-stone-600 mb-5">Upcoming Bookings</h1>
